...
export default function BenchmarkComparisonCard({
  ...
  const renderTiltRow = (row: GroupDetailRow) => (
    <div
      key={row.label}
      className="rounded-2xl border border-[--color-border-subtle] bg-[--color-background] px-3 py-2 shadow-sm"
    >
  ...
  const TiltColumn = ({ title, rows, emptyText }: TiltColumnProps) => (
    <div className="space-y-2">
      <p className="text-[0.65rem] font-semibold uppercase tracking-wide text-[--color-text-muted]">
        {title}
      </p>
      {rows.length ? (
        <div className="space-y-2">
        ...
        <p className="text-xs text-[--color-text-muted]">{emptyText}</p>
      )}
    </div>
  );

  return (
    <section className="space-y-4 rounded-3xl border border-[--color-border-subtle] bg-[--color-muted] p-5 shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex flex-col items-start gap-1 text-xs">
          <label className="text-[--color-text-muted]">Benchmark</label>
            <div className="relative inline-flex w-full min-w-[150px] rounded-full border border-[--color-border-subtle] bg-[--color-background] px-3 py-1 text-left text-sm font-semibold text-[--color-foreground] shadow-sm transition hover:bg-[--color-muted]">
              <select
                value={benchmark.id}
                onChange={handleChange}
                className="appearance-none w-full bg-transparent text-left text-sm font-semibold text-[--color-foreground] outline-none"
              >
            ...
            <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-xs text-[--color-text-muted]">
              ▾
            </span>
          </div>
        </div>
      ...
      <div className="space-y-2 text-xs sm:text-sm text-[--color-text-muted]">
        ...
        <div className="h-2 overflow-hidden rounded-full bg-[--color-muted-strong]">
          <div
      ...
      <div className="rounded-2xl border border-[--color-border-subtle] bg-[--color-background] p-4 shadow-sm">
...
function ComparisonList({
  ...
  return (
    <div className="space-y-2 rounded-2xl border border-[--color-border-subtle] bg-[--color-background] p-3 text-sm">
      ...
      {items.length ? (
        <ul className="space-y-1">
          {items.map((item) => (
            <li
              key={item.ticker}
              className="flex items-center justify-between font-medium text-[--color-foreground]"
            >
