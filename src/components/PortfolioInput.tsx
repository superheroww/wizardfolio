  ...
  return (
    <div className="relative">
      <input
        ...
        className="mt-1 w-full rounded-lg border border-[--color-border-subtle] bg-[--color-background] px-3 py-1.5 text-base sm:text-sm text-[--color-foreground] outline-none transition focus:border-[--color-accent]"
      />
      {dropdownVisible && (
        <div className="absolute left-0 right-0 z-10 mt-1 max-h-40 overflow-auto rounded-lg border border-[--color-border-subtle] bg-[--color-background] text-sm shadow-lg">
          {filteredSymbols.length > 0 ? (
            filteredSymbols.map((symbol) => (
              <button
                ...
                className="w-full cursor-pointer px-3 py-1.5 text-left text-sm text-[--color-foreground] hover:bg-[--color-muted]"
              >
                {symbol}
              </button>
            ))
...
export default function PortfolioInput({
  ...
  return (
    <section className="w-full rounded-2xl border border-[--color-border-subtle] bg-[--color-muted] p-4 shadow-sm">
      ...
      <div className="space-y-2">
        {positions.map((pos, index) => (
          <div
            key={index}
            className="flex w-full flex-nowrap items-end gap-3 rounded-xl border border-[--color-border-subtle] bg-[--color-background] p-3"
          >
            ...
            <div className="flex flex-col gap-1 w-24 min-w-[80px]">
              ...
              <input
                className="mt-1 w-full rounded-lg border border-[--color-border-subtle] bg-[--color-background] px-3 py-1.5 text-base sm:text-sm text-[--color-foreground] outline-none ring-0 focus:border-[--color-accent]"
                type="number"
                min={0}
                max={100}
                step={0.1}
            ...
            <button
              ...
              className="inline-flex h-8 w-8 flex-none items-center justify-center rounded-full border border-[--color-border-subtle] text-xs text-[--color-text-muted] hover:bg-[--color-muted]"
            >
              ×
            </button>
          </div>
      ...
      <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap gap-2">
          <button
            ...
            onClick={addRow}
            disabled={!canAddMore}
            className="inline-flex items-center rounded-full border border-[--color-border-subtle] bg-[--color-background] px-3 py-1 text-xs font-medium text-[--color-foreground] hover:bg-[--color-muted] disabled:cursor-not-allowed disabled:opacity-40"
          >
          ...
          <button
            ...
            onClick={onAnalyze}
            disabled={!canAnalyze}
            className="inline-flex items-center rounded-full bg-[--color-accent] px-4 py-1.5 text-xs font-semibold text-white disabled:opacity-40"
          >
            {analyzeLabel}
          </button>
        </div>
